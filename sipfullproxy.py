...
codes = {
    '200': '200 OK VSETKO V PORIADKU',
    '400': '400 ZLY FORMAT REQUESTU',
    '406': '406 NEAKCEPTOVATELNE',
    '480': '480 DOCASNE NEDOSTUPNE',
    '500': '500 CHYBA SERVERU'
}
...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def getCSeq(self):
        for line in self.data:
            if rx_cseq.search(line):
                return line
        return ""

    def getCallID(self):
        for line in self.data:
            if rx_call_id.search(line):
                return line[9:]
        return ""
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or origin not in registrar:
            self.sendResponse(codes['400'])
            return
        destination = self.getDestination()

        logging.info('Call-ID:{} Hovor od {} pre {}'.format(self.getCallID(), origin, destination))
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse(codes['480'])
        else:
            self.sendResponse(codes['500'])
    ...
    def processNonInvite(self):
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                self.sendResponse(codes['406'])
        else:
            self.sendResponse(codes['500'])

    def log_call_cancel(self):
        origin = self.getOrigin()
        if len(origin) == 0 or origin not in registrar:
            self.sendResponse(codes['400'])
            return
        destination = self.getDestination()
        if len(destination) > 0:
            logging.info('Call-ID:{} Pouzivatel {} zrusil hovor s pouzivatelom {}'.format(self.getCallID(), origin, destination))

    def log_call_bye(self):
        origin = self.getOrigin()
        if len(origin) == 0 or origin not in registrar:
            self.sendResponse(codes['400'])
            return
        destination = self.getDestination()
        if len(destination) > 0:
            logging.info('Call-ID:{} Pouzivatel {} ukoncil hovor s pouzivatelom {}'.format(self.getCallID(), origin, destination))
            ...
            if origin in registrar:
                ...
                if re.compile("200 Ok").search(self.data[0]):
                    if re.compile("INVITE").search(self.getCSeq()):
                        logging.info('Call-ID:{} Pouzivatel {} prijal hovor od pouzivatela {}'.format(self.getCallID(), dest, origin))

                elif re.compile("603 Decline").search(self.data[0]):
                    logging.info('Call-ID:{} Pouzivatel {} odmietol hovor od pouzivatela {}'.format(self.getCallID(), dest, origin))
